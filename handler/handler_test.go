...
func TestMissingPostData(t *testing.T) {
	v := url.Values{}
	// empty string
	v.Set("originalURL", "")
	postData := v.Encode()
	...
	router.ServeHTTP(rw, req)
	if rw.Code != 500 {
		t.Fatalf("HTTP response code must be 500 for the homepage", rw.Code)
	}
}

func TestUnknownShortURL(t *testing.T) {
	expectedOriginalUrl := "http://www.golang.org"
	localhost := "The short URL is <a href=\"/"
	...
	router.ServeHTTP(rw, req)
	if rw.Code != 302 {
		t.Fatalf("HTTP response code must be 302 for the homepage", rw.Code)
	}
	if rw.Header().Get("Location") != expectedOriginalUrl {
		t.Fatalf("Incorrect Location field in HTTP redirect",
			rw.Header().Get("Location"))
